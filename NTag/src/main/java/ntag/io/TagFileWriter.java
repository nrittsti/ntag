...
public final class TagFileWriter {
	...
	private void updateWithGenericTag(final AudioFile file, final TagFile tagFile) {
		...
		if (tagFile.getRating() > -1) {
			updateTextField(tag, FieldKey.RATING, "" + RatingConverter.out(tagFile.getAudioFormat(), tagFile.getRating()));
		}
	...
	private void updateWithMP3Tag(final MP3File file, final TagFile tagFile) {
		...
		// Create ID3v1 Tag
		if (!isID3v11()) {
			JAudiotaggerHelper.removeID3v1Tag(file);
		} else {
			final ID3v1Tag v1tag = JAudiotaggerHelper.getID3v1Tag(file);
		...
		// Create ID3v2? Tag
		final AbstractID3v2Tag v2tag = JAudiotaggerHelper.getID3v2Tag(file, isID3v24());
	...
	private void updateID3v2Rating(final AudioFormat audioFormat, int rating, final AbstractID3v2Tag tag) {
		...
		int convertedRating = RatingConverter.out(audioFormat, rating);
