...
public class ItemChoiceController<T> extends AbstractDialogController<ItemChoiceViewModel<T>> {
  ...
  private final ListChangeListener<? super T> selectionListChangeListener;
  ...
  public ItemChoiceController() {
    selectionListChangeListener = (Change<? extends T> event) -> updateButtonState();
  }
  ...
  private void updateButtonState() {
    createItemButton.setDisable(getViewModel().isSingleSelection() && selectionListView.getItems().size() > 0);
  }
  ...
  private void handleFilterAction(final ActionEvent event) {
    getViewModel().getFilteredItems().setPredicate(item -> item.toString().toLowerCase().contains(filterTextField.getText().toLowerCase()));
  }
  ...
  private void handleCreateItemAction(final ActionEvent event) {
    ...
    if (textValue.length() > 0) {
      ...
      try {
        ...
        LOGGER.log(Level.SEVERE, Resources.get("ntag", "msg_string_conversion_error"), e);
        FxUtil.showException(Resources.get("ntag", "msg_string_conversion_error"), e);
        return;
      }
      if (item == null) {
        Alert alert = new Alert(AlertType.ERROR, Resources.get("ntag", "msg_string_conversion_error"), ButtonType.OK);
        alert.showAndWait();
        return;
      }
      if (!selectionListView.getItems().contains(item)) {
